...
namespace ZipSaber
{
    ...
    public class Plugin
    {
        ...
        // Tracks whether BSML UI has been registered for the current menu session.
        // Reset on each scene change so we re-register after every reload.
        private bool _bsmlRegistered = false;
        ...
        public void OnEnable()
        {
            ...
            // On BSML 1.6.x there is no MainMenuAwaiter — use SceneManager instead.
        ...
        // ── Scene change handler (replaces MainMenuAwaiter for BSML 1.6.x) ───────
        private void OnActiveSceneChanged(Scene oldScene, Scene newScene)
        {
            if (newScene.name != "MainMenu") return;

            // Reset flag so we re-register on every menu load (handles scene reloads
            // after settings changes, same as the old MainMenuAwaiter subscription).
            _bsmlRegistered = false;
            RegisterBSMLUI();
        }

        private void RegisterBSMLUI()
        {
            if (_bsmlRegistered) return;
            try
            {
                // BSML 1.6.x uses lowercase .instance, not .Instance
